...
For the in-submarine selected-player face only, `מיקי` and `דור` use opposite source-facing overrides from each other so both point toward the blue submarine nose during gameplay. This does not change their start-screen card, countdown portrait, or HUD fighter rendering.
