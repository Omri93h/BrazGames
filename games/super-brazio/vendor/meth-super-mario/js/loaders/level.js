...
function setupBehavior(levelSpec, level) {
    level.music.setTimedSwitch(
        levelSpec.musicSwitch?.trackName,
        levelSpec.musicSwitch?.afterSeconds,
        levelSpec.musicSwitch?.fadeOutSeconds,
    );
...
function setupCheckpoints(levelSpec, level) {
    ...
    level.currentCheckpoint = new Vec2(level.checkpoints[0].x, level.checkpoints[0].y);
    level.checkpointFlags = (levelSpec.checkpointFlags || []).map((flagSpec, index) => {
        const [x, y] = flagSpec.pos;
        const [spawnX, spawnY] = flagSpec.spawn || flagSpec.pos;
        return {
            index,
            pos: new Vec2(x, y),
            spawn: new Vec2(spawnX, spawnY),
            reached: false,
        };
    });
}
...
function setupEntities(levelSpec, level, entityFactory) {
    ...
    levelSpec.entities.forEach(({id, name, pos: [x, y], props}, index) => {
        const createEntity = entityFactory[name];
        if (!createEntity) {
            console.warn(`Skipping unknown entity ${name}`);
            return;
        }
